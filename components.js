      ...
      <div class="brand">
        <div class="brand-icon">
          <svg viewBox="0 0 48 48" width="26" height="26" aria-hidden="true">
            ...
            <path d="M24 3 C 30 10, 30 20, 24 24 C 18 28, 8 28, 4.5 24" fill="none" stroke="#0057FF" stroke-width="3.2" stroke-linecap="round"/>
            <path d="M24 3 C 18 10, 18 20, 24 24 C 30 28, 40 28, 43.5 24" fill="none" stroke="#D7FF00" stroke-width="3.2" stroke-linecap="round"/>
            <path d="M4.5 24 C 10 30, 20 34, 24 45 C 28 34, 38 30, 43.5 24" fill="none" stroke="#0057FF" stroke-width="3.2" stroke-linecap="round"/>
            <path d="M24 24 C 22 32, 22 40, 24 45" fill="none" stroke="#D7FF00" stroke-width="3" stroke-linecap="round"/>
...
// A small, non-blocking notification — for simple confirmations (logged in,
// saved, logged out) that don't need a decision from the user. Fades in from
// the top right, sits briefly, fades out on its own. Use showAlert/showConfirm
// instead when the user actually needs to read something or make a choice.
...
function renderCarousel(id, dotsId, itemCount, cardsHTML, emptyHTML, forceScroll){
  ...
  el.dataset.forceScroll = forceScroll ? "1" : "";
...
// overflows (or forceScroll was requested, e.g. Fixtures) do we duplicate
// it once, for a seamless auto-scroll loop.
function applyCarouselLayout(id, dotsId){
  ...
  requestAnimationFrame(()=>{
    ...
    if(singleWidth <= el.clientWidth + 2 && el.dataset.forceScroll !== "1"){
...
function showConfirm(message, onYes, onNo, yesLabel, noLabel){
  injectModal();
  document.getElementById("modalMessage").innerText = message;
  document.getElementById("modalActions").innerHTML = `<button class="btn" id="modalYesBtn">${yesLabel||"Yes"}</button><button class="btn btn-quiet-dark" id="modalNoBtn">${noLabel||"No"}</button>`;
  document.getElementById("modalOverlay").classList.add("open");
