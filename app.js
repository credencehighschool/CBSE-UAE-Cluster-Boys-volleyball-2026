...
function init(){subtitle.textContent=SITE_CONFIG.eventTitle+" "+SITE_CONFIG.eventYear;hostedBy.textContent=SITE_CONFIG.hostedBy;categories.textContent=SITE_CONFIG.categories;dates.textContent=SITE_CONFIG.championshipDates;registerBtn.href=SITE_CONFIG.registrationUrl;fix14.href=SITE_CONFIG.fixtures.u14;fix17.href=SITE_CONFIG.fixtures.u17;fix19.href=SITE_CONFIG.fixtures.u19;contactName.textContent=SITE_CONFIG.contact.name;contactRole.textContent=SITE_CONFIG.contact.role;contactEmail.textContent=SITE_CONFIG.contact.email;contactEmail.href="mailto:"+SITE_CONFIG.contact.email;contactMobile.textContent=SITE_CONFIG.contact.mobile;contactMobile.href="tel:"+SITE_CONFIG.contact.phoneLink;downloads.innerHTML=SITE_CONFIG.downloads.map(d=>`<div class="card"><h2>${d.icon}</h2><h3>${d.title}</h3><br><a class="btn" href="${d.url}" target="_blank">Open</a><br><img style="width:90px;margin-top:12px" src="${qr(d.url)}"></div>`).join("");venue.innerHTML=`<h2>${SITE_CONFIG.venue.title}</h2><p class="muted">${SITE_CONFIG.venue.details}</p><br><a class="btn" href="${SITE_CONFIG.venue.mapUrl}" target="_blank">Open Location</a><br><img style="width:90px;margin-top:12px" src="${qr(SITE_CONFIG.venue.mapUrl)}">`;countdown();loadNews()}
...
async function loadLive(){try{let d=await get("live");liveGrid.innerHTML=(d.live||[]).map(renderMatch).join("")||'<div class="card">No live matches now.</div>';timers()}catch(e){liveGrid.innerHTML='<div class="card">Unable to load live matches.</div>'}}
